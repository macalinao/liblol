...
describe "items", ->
  ...
  describe "find", ->
    ...
    it "should return all items when nothing is passed to it", ->
      items.find().should.include item for item in items._list

    it "should get the item by name if a string is passed", ->
      items.find("Hextech Gunblade").should.eql [items._list["Hextech Gunblade"]]

    it "should use the 'where' filter if an object is passed", ->
      items.find(name: "Hextech Gunblade").should.eql [items._list["Hextech Gunblade"]]
    ...
    describe "filters", ->
      ...
      describe "and", ->
        ...
        it "should include only cases that fulfill both filters", ->
          ...
          list.should.include items._list["Hextech Gunblade"] # ap and ad
          list.should.not.include items._list["Hextech Revolver"] # ap
          list.should.not.include items._list["Bilgewater Cutlass"] # ad
      ...
      describe "or", ->
        ...
        it "should include cases that fulfill any filters", ->
          ...
          list.should.include items._list["Hextech Gunblade"] # ap and ad
          list.should.include items._list["Hextech Revolver"] # ap
          list.should.include items._list["Bilgewater Cutlass"] # ad
          list.should.not.include items._list["Warmog's Armor"] # none
      ...
      describe "where", ->
        ...
        it "should check for equality if the parameter isn't a query object", ->
          items.find(where name: "Hextech Gunblade").should.eql [items._list["Hextech Gunblade"]]

        it "should check for equality with the 'eq' filter", ->
          items.find(where name: eq: "Hextech Gunblade").should.eql [items._list["Hextech Gunblade"]]

        it "should check for >= with the 'gte' filter", ->
          items.find(where name: "Hextech Gunblade", ap: gte: 65).should.eql [items._list["Hextech Gunblade"]]
          items.find(where name: "Hextech Gunblade", ap: gte: 66).should.eql []

        it "should check for > with the 'gt' filter", ->
          items.find(where name: "Hextech Gunblade", ap: gt: 64).should.eql [items._list["Hextech Gunblade"]]
          items.find(where name: "Hextech Gunblade", ap: gt: 65).should.eql []

        it "should check for <= with the 'lte' filter", ->
          items.find(where name: "Hextech Gunblade", ap: lte: 65).should.eql [items._list["Hextech Gunblade"]]
          items.find(where name: "Hextech Gunblade", ap: lte: 64).should.eql []

        it "should check for < with the 'lt' filter", ->
          items.find(where name: "Hextech Gunblade", ap: lt: 66).should.eql [items._list["Hextech Gunblade"]]
          items.find(where name: "Hextech Gunblade", ap: lt: 65).should.eql []

        it "should work with multiple query filters applied at once", ->
          items.find(where name: "Hextech Gunblade", ap: lt: 66, gt: 54).should.eql [items._list["Hextech Gunblade"]]
          items.find(where name: "Hextech Gunblade", ap: lt: 69, gt: 65).should.eql []

      describe "withStats", ->

        it "should not include items without the stat", ->
          items.find(items.filters.withStats(["ap"])).should.not.include items._list["B. F. Sword"]

        it "should include items with the stat", ->
          items.find(items.filters.withStats(["ap"])).should.include items._list["Needlessly Large Rod"]

        it "should include items with multiple stats", ->
          items.find(items.filters.withStats(["ap", "ad"])).should.include items._list["Hextech Gunblade"]
